...
// The "no card" state for a profile route — a soft 404 / rate-limit / error
// screen. Presentational only (no hooks), so it renders from either a server or
// a client component. The error is passed as a plain shape so no server-only
// module is pulled into the client bundle.
export default function NotScouted({
  username,
  error,
}: {
  username: string;
  ...
  const message = rateLimited
    ? `You lot went viral and stormed the training ground all at once, so GitHub showed us a yellow card for time-wasting. Give the scouts a couple minutes to catch their breath, then send @${username} back on.`
    : error.type === "notfound"
